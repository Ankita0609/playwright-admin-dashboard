...
tests/
├── auth.setup.ts # Auth via storageState (login once)
├── admin-login.spec.ts # Dashboard access validation
├── admin-user-management.spec.ts
├── admin-create-user.spec.ts
└── pages/
└── AdminPage.ts # Page Object abstraction

storage/
└── admin.json # Auth session storage

Jenkinsfile # Windows-safe Jenkins CI
...
# 2️⃣ JENKINS BADGE (ADD TO README TOP)

Once your Jenkins job URL is stable, add this **at the top of README**:

```md
![Jenkins](http://localhost:8080/job/playwright-admin-dashboard/badge/icon)
If Jenkins is remote later, update the URL.
